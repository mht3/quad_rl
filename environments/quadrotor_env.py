...
class QuadrotorEnv(gym.Env):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--time_per_waypoint', type=float, default=2.5)
    ...
    def generate_random_waypoints(self, num_waypoints=2):
        '''
        Generates `num_waypoints` random waypoints in x, y, z plane
        '''
        low_range = np.array([-0.25, -0.25, 0.25], dtype=np.float32) * self.boundary_length
        high_range = np.array([ 0.25,  0.25, 0.75], dtype=np.float32) * self.boundary_length
        waypoints = np.zeros((num_waypoints, 4))
        waypoints[:, :3] = np.random.rand(num_waypoints, 3) * (high_range - low_range) + low_range
        return waypoints
